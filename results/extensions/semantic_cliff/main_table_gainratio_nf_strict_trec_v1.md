...
- NFCorpus strict v1 is generated from the newly completed strict pipeline: wm_prepare → wmunit → doc → inject → boundary.
- NFCorpus offline v4 should no longer be used in the main table. It can be moved to appendix or deleted from the main result discussion.
- GainRatio is the main method. DeltaLoss is retained as an ablation/intermediate score.
- For both NFCorpus and TREC-COVID, GainRatio substantially outperforms Contriever + Cosine, especially under low false-positive-rate settings.
