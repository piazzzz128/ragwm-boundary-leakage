...
IN_GLOB = "/root/autodl-tmp/ragwm_storage/output/adaptive_attack/boundary_*_attack_a*_qwen-plus_v2.json"
OUT_DIR = Path("/root/autodl-tmp/ragwm_storage/output/adaptive_attack/cleaned")
OUT_DIR.mkdir(parents=True, exist_ok=True)

SUMMARY_JSON = OUT_DIR / "adaptive_attack_qwen_plus_v2_clean_summary.json"
SUMMARY_MD = OUT_DIR / "adaptive_attack_qwen_plus_v2_clean_summary.md"
...
def clean_one(path):
    ...
    dataset, attack = parse_name(path)
    ...
    for full_text, rows in groups.items():
        ...
        # Prefer keeping inject if duplicate crosses labels; then rebalance later.
    ...
    for i, r in enumerate(balanced):
        ...
        r["id"] = f"{dataset.replace(' ', '_').lower()}_{attack}_cleaned_{prefix}_{i}"
...
def main():
    ...
    paths = [p for p in paths if "/cleaned/" not in p and not p.endswith("_audit.json")]
    ...
    with open(SUMMARY_MD, "w", encoding="utf-8") as f:
        f.write("# Adaptive Attack Cleaned Boundary Summary\n\n")
        f.write("| Dataset | Attack | Before N | Dup Groups | Removed | Final N | Labels | target_in_context | duplicate_full_text |\n")
        f.write("|---|---|---:|---:|---:|---:|---|---:|---:|\n")
        for a in audits:
            f.write(
                f'| {a["dataset"]} | {a["attack"]} | {a["before_n"]} | '
