...
OUT_DIR=Path("/root/autodl-tmp/ragwm_storage/output/adaptive_attack/cleaned")
SCORES_PATH=OUT_DIR/"adaptive_attack_qwen_plus_v2_clean_matched_aligned_scores.json"
METRICS_PATH=OUT_DIR/"adaptive_attack_qwen_plus_v2_clean_matched_aligned_metrics.json"
CACHE_PATH=OUT_DIR/"adaptive_attack_qwen_plus_v2_clean_matched_aligned_score_cache.json"
MD_PATH=OUT_DIR/"adaptive_attack_qwen_plus_v2_clean_matched_aligned_table.md"
CSV_PATH=OUT_DIR/"adaptive_attack_qwen_plus_v2_clean_matched_aligned_table.csv"
...
def score_one(context, target, model, tokenizer, device):
    ...
    # aligned scoring: L(T) predicts first target token using EOS prefix
...
def main():
    ...
    if not paths:
        raise RuntimeError("No cleaned adaptive attack files found.")
    ...
    for p in paths:
        ...
        for variant in ["original_matched", "attacked"]:
            ...
            print("\nEVAL", dataset, attack, variant, "n=", len(rows))
            ...
            for i,r in enumerate(rows):
                ...
                if (i+1)%100==0:
                    print(dataset, attack, variant, "scored", i+1, "/", len(rows), "cache", len(cache), flush=True)
            ...
            print(dataset, attack, variant, "GainRatio", metrics(scored,"gain_anomaly_score"))
    ...
    with open(MD_PATH,"w",encoding="utf-8") as f:
        f.write("# Adaptive Paraphrase Attack Robustness: qwen-plus v2 Clean Matched\n\n")
        f.write("## GainRatio Main Results\n\n")
        f.write("| Dataset | Attack | Variant | N | ROC-AUC | PR-AUC | TPR@FPR=5% | TPR@FPR=10% | Best F1 |\n")
        f.write("|---|---|---|---:|---:|---:|---:|---:|---:|\n")

        for ds in ["NFCorpus","TREC-COVID","Natural Questions"]:
            b=EMBEDDING_BASELINE[ds]
            f.write(f'| {ds} | baseline | Contriever+Cosine | {b["n"]} | {b["roc_auc"]:.4f} | {b["pr_auc"]:.4f} | {b["tpr_at_fpr_5"]:.4f} | {b["tpr_at_fpr_10"]:.4f} | {b["best_f1"]:.4f} |\n')
            for attack in ["a1","a2","a3"]:
                for variant in ["original_matched","attacked"]:
                    rr=[r for r in all_metrics if r["dataset"]==ds and r["attack"]==attack and r["variant"]==variant and r["method"]=="GainRatio"]
                    if rr:
                        r=rr[0]
                        f.write(f'| {ds} | {attack} | {variant} | {r["n"]} | {r["roc_auc"]:.4f} | {r["pr_auc"]:.4f} | {r["tpr_at_fpr_5"]:.4f} | {r["tpr_at_fpr_10"]:.4f} | {r["best_f1"]:.4f} |\n')
