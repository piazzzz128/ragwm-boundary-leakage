...
def encode_texts(model, tokenizer, texts, device):
    ...
    # Different Contriever/BERT wrappers return different structures:
    # 1) Tensor [batch, hidden]                     -> use directly
    # 2) Object with last_hidden_state [batch, seq, hidden] -> mean pooling
    # 3) tuple/list whose first item is [batch, hidden] or [batch, seq, hidden]
    ...
    if emb.dim() != 2:
        raise RuntimeError(f"Expected embedding shape [batch, hidden], got {tuple(emb.shape)}")
