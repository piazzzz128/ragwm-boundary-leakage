...
def target_nll_aligned(
    ...
    label_positions = list(range(start + 1, len(input_ids)))
...
def main() -> None:
    ...
    parser.add_argument("--model-name", default="DeepSeek-LLM-7B-base")
    parser.add_argument(
    ...
    manifest = {
        ...
        "warning": (
            "Cross-detector extension only; do not overwrite Qwen or sealed "
            "strict-v1 scores."
        ),
    }
    ...
    if complete:
        print(f"DEEPSEEK {args.dataset.upper()} ALIGNED SCORING PASS")
