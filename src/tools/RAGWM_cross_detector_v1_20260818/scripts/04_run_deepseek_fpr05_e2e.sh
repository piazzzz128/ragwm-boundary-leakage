...
PACKAGE_DIR=${PACKAGE_DIR:-$REPO/tools/RAGWM_cross_detector_v1_20260818}
...
OUT_ROOT=${OUT_ROOT:-$STORAGE/output/cross_detector_lm_robustness_v1/deepseek_llm_7b_base}
MODEL_PATH=${MODEL_PATH:-$STORAGE/local_models/DeepSeek-LLM-7B-base-ms}

THRESHOLD="$OUT_ROOT/thresholds/deepseek_fpr05.json"
PREFLIGHT="$OUT_ROOT/audit/preflight.json"
CONDITION="$OUT_ROOT/conditions/deepseek_llm_7b_base_fpr05"
DB="$OUT_ROOT/vectorstores/deepseek_llm_7b_base_fpr05"
...
if threshold.get("model") != "DeepSeek-LLM-7B-base":
    raise SystemExit("[ERROR] Threshold model mismatch")
print("DEEPSEEK FORMAL INPUT GUARDS PASS")
PY
...
sha256sum \
  "$PREFLIGHT" \
  "$OUT_ROOT/scores/deepseek_nfcorpus_aligned_scores.json" \
  "$OUT_ROOT/scores/deepseek_trec_aligned_scores.json" \
  "$OUT_ROOT/metrics/deepseek_boundary_metrics.json" \
  "$THRESHOLD" \
...
echo "DEEPSEEK CROSS-DETECTOR FORMAL E2E PASS"
