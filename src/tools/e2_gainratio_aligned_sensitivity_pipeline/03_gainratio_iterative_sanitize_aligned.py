...
def process_document(
    ...
    for step in range(1, max_steps + 1):
        ...
        scoring = gainratio_score_aligned(
            tokenizer,
            model,
            device,
            context,
...
def compile_outputs(
    ...
    total_original_chars = sum(
        row["original_char_count"] for row in records
    )
    total_deleted_chars = sum(
        row["deleted_char_count"] for row in records
    )
    modified_count = sum(bool(row["modified"]) for row in records)
    ...
    summary = {
        ...
        "modified_document_rate": (
            modified_count / len(records) if records else None
        ),
        "candidate_evaluation_count": len(candidate_rows),
        "deletion_step_count": sum(
            row["deletion_step_count"] for row in records
        ),
        "total_original_chars": total_original_chars,
        "total_deleted_chars": total_deleted_chars,
        "deleted_char_rate": (
            total_deleted_chars / total_original_chars
            if total_original_chars
            else None
        ),
        "stop_reason_counts": dict(
            Counter(row["stop_reason"] for row in records)
        ),
        ...
        "warning": (
            "Sensitivity output only. Do not overwrite or rename as the "
            "sealed strict-v1 E2 main result."
        ),
    }
    write_json_atomic(
        output_dir / "gainratio_sanitization_summary.json",
        summary,
    )
    return summary
...
def main() -> None:
    ...
    if threshold_report.get("experiment_role") != (
        "aligned_end_to_end_sensitivity"
    ):
        raise RuntimeError(
            "Threshold JSON is not marked as aligned sensitivity"
        )
    ...
    print("Scoring version: aligned_minimal_v1")
    ...
    if leaked:
        raise RuntimeError(
            f"Forbidden ground-truth fields in attack input: {leaked}"
        )
    ...
    if args.limit == 0 and not summary["complete"]:
        raise RuntimeError("Formal aligned sanitization is incomplete")

    print("ALIGNED GAINRATIO SANITIZATION PASS")
