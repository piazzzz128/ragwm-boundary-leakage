# RAG-WM Qwen--DeepSeek 配对统计包 v2（2026-08-19）

## v2 修正说明

v1 错把 `boundary_trec_k5_v3.json`（TREC 边界构造输入）当成 Qwen 已评分输出，因此其记录不含 `score_aligned`。v2 直接携带权威交接包中的 `inputs/trec_aligned_qwen_v1_scores.json`，并以 SHA-256 固定。该修正不改变、重跑或推算任何实验结果。
...
```text
/root/autodl-tmp/ragwm_storage/repo/ragwm_src/tools/RAGWM_pairwise_qwen_deepseek_v2_20260819
```

然后运行：

```bash
cd /root/autodl-tmp/ragwm_storage/repo/ragwm_src/tools/RAGWM_pairwise_qwen_deepseek_v2_20260819
chmod +x scripts/*.py
...
export PAIR_OUT=$STORAGE/output/cross_detector_lm_robustness_v1/pairwise_qwen_deepseek_v2

export QWEN_NF=$STORAGE/output/semantic_cliff/aligned_sensitivity/nfcorpus_aligned_scores.json
export QWEN_TREC=$PWD/inputs/trec_aligned_qwen_v1_scores.json
...
- TREC-COVID Qwen 已评分输出必须匹配冻结 SHA-256 `cc6a22420492cf63ddcaa1be90d2528db907912f905ce2f4425e26a894aed4e2`。
