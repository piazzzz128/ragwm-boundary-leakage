...
from e2_common import sha256_file, write_json_atomic
...
def main() -> None:
    ...
    for row in clean:
        ...
        clean_groups[group].append(float(row["score"]))

    calibration_groups = {
        group for group in clean_groups if bucket(group, args.seed) < args.calibration_fraction
    }
    ...
    inject_scores = np.array([float(row["score"]) for row in inject], dtype=float)
    report = {
        "policy": "strict-v1 clean calibration score quantile",
        ...
        "strict_scores_path": args.scores,
        "strict_scores_sha256": sha256_file(Path(args.scores)),
        "warning": (
            "Threshold is fixed before E2 verification. Do not retune it using E2 WSN. "
            "The uploaded strict-v1 scorer is reproduced exactly and is not an aligned-first-token implementation."
        ),
    }
    output = Path(args.output)
    write_json_atomic(output, report)
    print(json.dumps(report, ensure_ascii=False, indent=2))
    print("THRESHOLD CALIBRATION PASS")
