...
def sentence_spans(text: str) -> List[Tuple[int, int]]:
    """Return sentence-like spans while preserving positions in the original text."""
    if not text:
        return []
...
@torch.no_grad()
def target_nll_exact_strict_v1(tokenizer, model, device: str, context: str, target: str):
    """Exact scoring behavior from nfcorpus_strict_qwen_gainratio.py.

    This intentionally preserves the original strict-v1 implementation for
    comparability, including its first-target-token asymmetry.
    """
    ...
    for position in range(start, end):
        if position == 0:
            continue
        pred_positions.append(position - 1)
    ...
    loss = torch.nn.functional.cross_entropy(
        pred_logits.float(), labels, reduction="mean"
    )
    return float(loss.item())


def gainratio_score_exact_strict_v1(tokenizer, model, device: str, context: str, target: str):
    loss_target = target_nll_exact_strict_v1(tokenizer, model, device, "", target)
    loss_conditioned = target_nll_exact_strict_v1(
