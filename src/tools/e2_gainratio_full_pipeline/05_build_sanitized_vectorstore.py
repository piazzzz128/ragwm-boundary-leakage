...
from e2_common import read_jsonl, write_json_atomic
...
def main() -> None:
    ...
    if len(rows) != 3633:
        raise RuntimeError(f"Expected 3633 sanitized documents, found {len(rows)}")
    ...
    existing_names = [getattr(item, "name", str(item)) for item in client.list_collections()]
    ...
    for start in range(0, len(rows), args.batch_size):
        ...
        metadatas = [
            {
                ...
                "title": row.get("title", ""),
                "change": False,
                "sanitization": "gainratio_iterative_suffix",
            }
            for row in batch
        ]
        collection.add(
        ...
        print(f"embedded {min(start + len(batch), len(rows))}/{len(rows)}", flush=True)
    ...
    write_json_atomic(db_path.parent / "e2_vectorstore_build_summary.json", report)
    print(json.dumps(report, ensure_ascii=False, indent=2))
    if count != len(rows):
        raise RuntimeError(f"Vector store count mismatch: {count} != {len(rows)}")
    print("E2 VECTORSTORE BUILD PASS")
