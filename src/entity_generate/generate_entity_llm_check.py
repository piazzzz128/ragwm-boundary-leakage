import os as _ragwm_safe_os, sys as _ragwm_safe_sys
_ragwm_safe_sys.path.insert(0, _ragwm_safe_os.path.dirname(__file__))
import safe_llm_patch  # RAGWM_SAFE_PATCH
...
def _bad_entity_name(x: str) -> bool:
    import re
    if x is None:
        return True
    x = str(x).strip()
    ...
    bad_exact = {
        ...
        "tip", "objectivity", "longino", "daston", "galison",
        "survey", "supplementary_material", "supplementary material",
    ...
    bad_kw = [
        ...
        "beyond its borders", "operational efficiency",
        "amount of training data", "real subjects",
        "reformist seasons", "university of example",
        "climate change", "anthropogenic activity"
    ]
    if any(k in xl for k in bad_kw):
        return True

    if re.fullmatch(r"[\d,\.\-\s]+", x):
        return True
    if re.fullmatch(r"[\d,\.\-\s]+died", xl):
        return True
    if len(x) < 3 or len(x) > 120:
        return True
...
if __name__ == '__main__':
    ...
    base_url = api_base[api_pos] if isinstance(api_base, list) else api_base
    model_name = config["model_info"]["name"]

    assert isinstance(base_url, str) and base_url.startswith(("http://", "https://")), f"Invalid base_url: {base_url!r}"

    os.environ["OPENAI_API_KEY"] = api_key
    os.environ["OPENAI_API_BASE"] = base_url
    os.environ["OPENAI_BASE_URL"] = base_url

    print("USING_MODEL:", model_name, flush=True)
    print("USING_BASE_URL:", base_url, flush=True)

    llm = ChatOpenAI(
        http_client=httpx.Client(http2=False, timeout=120.0, verify=False),
        max_retries=1,
        timeout=120,
        temperature=0,
